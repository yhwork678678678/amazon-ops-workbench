...
- 备忘录：保存运营事项到当前浏览器的 localStorage，可新增、编辑、删除。
- 页面启动时会后台检查 GitHub Pages 的构建入口；发现新版本资源后自动刷新，不会清除浏览器本地数据。
...
- 参考 Linear 设计语言的暗色系统：近黑画布 `#010102` + 表面阶梯 + hairline 细边框分层。
- 文字层级：`#f7f8f8`（标题）/ `#d0d6e0`（正文）/ `#8a8f98`（说明）。
- Lavender-Blue `#5e6ad2` 作为唯一彩色 accent（主操作、导航激活、时间线标记）。
- 语义色：绿 `#27a644`（正利润）、红 `#eb5757`（负利润/删除）、黄 `#f2c94c`（提醒）。
- 卡片圆角控制在 8px 内，层次靠边框和明度区分。
- 使用 lucide 图标辅助按钮识别。

当前布局：

- 桌面端左侧固定导航，右侧工作区。
- 工作区顶部左侧显示日期和星期，右侧按需显示通知入口。
- 备忘录位于主内容前部（卡片带青绿色左边线），后面依次是概览、利润测算和上传文件区。
- 原底部"下一步计划"文案收纳为侧栏底部小字。
- 移动端改为单列布局，导航吸顶常驻（小屏为两列按钮，品牌区在小屏隐藏）。

当前交互细节：

- 备忘时间线按天分组，组标签为"今天/昨天/日期"；标签按语义配色（广告=琥珀、库存=红、调价=蓝、复盘=灰、Listing=青绿）。
- 备忘正文保留换行显示；备忘清空后时间线显示空状态提示。
- 编辑备忘时，被编辑的时间线卡片青绿色高亮。
- 巡检列表按时间升序排列，时间下方显示提醒徽标；行悬停时显示编辑/删除按钮，编辑中的行高亮。
- 利润结果区主数字放大强调，负利润显示淡红背景。
- 上传密钥栏通过标题栏按钮展开/收起，无密钥时默认展开；上传中显示进度。
- 文件行的仓库内部路径仅悬停时显示。
...
备忘录界面由 `NotesPanel` 组件独立维护，使用“快速输入 + 时间线记录”的结构。新增备忘的创建时间默认是当前时间，也可以手动调整；时间线中的备忘可点击编辑按钮回填表单并修改创建时间，保存修改或取消编辑。时间线按天分组显示（今天/昨天/日期标签），标签按语义配色。
...
页面支持新增、编辑、删除和恢复默认流程。列表每行在时间下方显示提醒徽标（铃铛 + 提前分钟数）；行悬停时显示编辑/删除按钮（移动端常显），编辑中的行高亮。数据只保存在当前浏览器，不会上传到 GitHub。
