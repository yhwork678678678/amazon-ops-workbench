...
- `src/components/NotesPanel.tsx`：备忘录输入、时间线展示和删除操作。
...
备忘录界面由 `NotesPanel` 组件独立维护，使用“快速输入 + 时间线记录”的结构。时间线中的备忘可点击编辑按钮回填表单，保存修改或取消编辑；后续可在组件内部增加搜索、标签筛选和归档视图。
