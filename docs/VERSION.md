...
当前项目已完成静态 MVP，文件区仅保留私密 GitHub 上传、预览和下载。
...
- 左侧品牌区只保留“工作台”。
- 删除 `AMZ` 图标和 `Personal console` 文案。
...
- 新上传文件名只保留 `YYYY-MM-DD` 日期前缀。
- 移除上传文件名中的时间戳和随机编号。
- 保留历史文件原有名称，避免影响已有文件链接。
...
- 移除浏览器 IndexedDB 临时文件区。
- 文件区仅保留私密 GitHub 上传文件区。
- 保留私密文件的刷新、预览和下载能力。
